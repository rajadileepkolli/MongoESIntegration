...
public class ElasticSearchOperationsTest extends MongoESConfigTest {
  ...
  public final void testPerformElasticSearch() throws DigitalBridgeException {
    ...
    assertNotNull(response.getContent().get(0).getAddress().getLocation().getCoordinates());
  ...
  @Test
  public final void testTermFacetSearchWithOutFilters() throws DigitalBridgeException, IOException {
    SecurityUtils.runAs(USERNAME, PASSWORD, ROLE_USER);
  ...
  @Test
  public final void testTermFacetSearchWithFilters() throws DigitalBridgeException, IOException {
    SecurityUtils.runAs(USERNAME, PASSWORD, ROLE_USER);
    ...
    facetDateRange.setStartDate(
        DateTime.parse("2014-12-01T00:00:00.000Z", ISODateTimeFormat.dateTimeParser().withOffsetParsed()));
    facetDateRange
        .setEndDate(DateTime.parse("2014-12-31T23:59:59.999Z", ISODateTimeFormat.dateTimeParser().withOffsetParsed()));
    FacetDateRange facetDateRange1 = new FacetDateRange();
    facetDateRange1.setStartDate(
        DateTime.parse("2015-04-01T00:00:00.000Z", ISODateTimeFormat.dateTimeParser().withOffsetParsed()));
    facetDateRange1
        .setEndDate(DateTime.parse("2015-04-30T23:59:59.999Z", ISODateTimeFormat.dateTimeParser().withOffsetParsed()));
    termFilters.put("lDate", new Object[] { facetDateRange, facetDateRange1 });
    Map<String, Map<String, Long>> response = elasticSearchOperations.termFacetSearch(termFilters, true);
    assertTrue(!response.isEmpty());
    assertTrue(response.size() == 3);
  }
